...
const renderSubcategory = ({ category }) => {
  let href = `user/browse-categories/${encodeURIComponent(category.category_id.categoryName)}`;
  return (
  ...
  const fetchSubcategories = async () => {
    try {
      if (selectedLocation && selectedLocation._id) {
        const { data } = await axios.post(`/api/fetch-looking-for`, { id: selectedLocation._id });
